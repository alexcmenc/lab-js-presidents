...
function countYearsInOffice(presidentsArr) {
  return presidentsArr.reduce(
    (acc, x) =>
      x.leftOffice === null ? acc : x.leftOffice - x.tookOffice + acc,
    0
  );
}
...
function getAgeAtInauguration(presidentsArr) {
  return presidentsArr.map((x) => {
    x.ageAtInauguration = x.tookOffice - x.birthYear;
    return x;
  });
}
...
function getPresidentsBornAfter(presidentsArr, year) {
  return presidentsArr.filter(
    (presAfterYear) => presAfterYear.birthYear > year
  );
}
...
function countRepublicanPresidents(presidentsArr) {
  return presidentsArr.reduce(
    (acc, z) => (z.party === "Republican" ? acc + 1 : acc),
    0
  );
}
